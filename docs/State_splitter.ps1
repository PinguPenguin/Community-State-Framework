        ...
        Get-Content -Path $NewSource |
            ForEach-Object {
                ...
                elseif ((-not $FileOpen) -and
                        ...
                        ([regex]::Matches($Line.Substring(0, 6), [ref] $StateStart)))
                {
                    if ($StateCount -lt 10) { $StatePad = "0"; }
                    else { $StatePad = ""; }
                    $Parts = $Line.Split(" ")
                    $NewFileName = [string]::Concat($OutputDirectory, $FilePrefix, "_", $StatePad, $StateCount, "_", $Parts[0], ".txt");
